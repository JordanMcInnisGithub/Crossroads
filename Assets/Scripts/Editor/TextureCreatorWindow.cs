...
{
	string filename = "myProcedualTexture";
	float perlinXScale;
	float perlinYScale;
	int perlinOctaves;
			...
			for (int y = 0; y < h; y++)
			{
				for (int x = 0; x < w; x++)
				{
					if (seamlessToggle)
					{
						...
						float noise00 = TerrainUtils.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
						float noise01 = TerrainUtils.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY + h) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
						float noise10 = TerrainUtils.fBM((x + perlinOffsetX + w) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
						float noise11 = TerrainUtils.fBM((x + perlinOffsetX + w) * perlinXScale, (y + perlinOffsetY + h) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
					...
					{
						pValue = TerrainUtils.fBM((x + perlinOffsetX) * perlinXScale, (y + perlinOffsetY) * perlinYScale, perlinOctaves, perlinPersistance) * perlinHeightScale;
					}
			...
			if (mapToggle)
			{
				for (int y = 0; y < h; y++)
				{
					for (int x = 0; x < w; x++)
					{
						...
						colValue = TerrainUtils.Map(colValue, minColour, maxColour, 0, 1);
						pixCol.r = colValue;
						pixCol.g = colValue;
						pixCol.b = colValue;
						pTexture.SetPixel(x, y, pixCol);
